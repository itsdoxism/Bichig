...
def clean_texts(texts: list[str], min_chars: int = 20, max_chars: int = 1000, min_cyrillic_ratio: float = 0.75) -> tuple[list[str], dict]:
    seen = set(); clean = []
    stats = {"raw": len(texts), "empty": 0, "too_short": 0, "too_long": 0, "low_cyrillic": 0, "duplicates": 0, "clean": 0}
    for raw in texts:
        text = normalize_text(raw)
        if not text: stats["empty"] += 1; continue
        if len(text) < min_chars: stats["too_short"] += 1; continue
        if len(text) > max_chars: stats["too_long"] += 1; continue
        if cyrillic_letter_ratio(text) < min_cyrillic_ratio: stats["low_cyrillic"] += 1; continue
        key = text.casefold()
...
def split_texts(texts: list[str], valid_ratio: float, seed: int) -> tuple[list[str], list[str]]:
    items = list(texts); random.Random(seed).shuffle(items)
    if not items: return [], []
    if valid_ratio <= 0 or len(items) == 1: return items, []
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--allow-unknown-license", action="store_true", help="allow enabled research-only sources whose license is unknown")
...
def main() -> None:
    a = parse_args(); raw = []; per_source = {}; source_meta = []; input_paths = list(a.inputs)
    if a.manifest:
        for rec in load_manifest(a.manifest):
            ...
            source_meta.append({"name": rec.name, "path": rec.path, "license": rec.license, "kind": rec.kind, "language": rec.language, "usage": rec.usage, "redistribute": rec.redistribute})
    if not input_paths: raise SystemExit("provide at least one input path or --manifest")
    for name in input_paths:
        path = Path(name); rows = iter_texts(path, a.field); per_source[str(path)] = len(rows); raw.extend(rows)
    ...
    out = Path(a.out); write_lines(out / "train.txt", train); write_lines(out / "valid.txt", valid)
    report = {"sources": per_source, "source_metadata": source_meta, **stats, "train": len(train), "valid": len(valid), "seed": a.seed, "valid_ratio": a.valid_ratio, "min_cyrillic_ratio": a.min_cyrillic_ratio}
