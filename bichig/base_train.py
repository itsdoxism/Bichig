...
def main():
    ...
    print(f"device={device} lines={len(texts)} tokens={len(ids)} vocab={len(tok)} params={sum(p.numel() for p in model.parameters()):,}")
    for epoch in range(1, a.epochs + 1):
        ...
        print(f"epoch={epoch:03d} loss={avg:.4f} ppl={math_exp(avg):.2f}")
        torch.save({"model": model.state_dict(), "config": cfg.to_dict(), "vocab": tok.itos, "epoch": epoch, "loss": avg}, out / "last.pt")
    torch.save({"model": model.state_dict(), "config": cfg.to_dict(), "vocab": tok.itos, "epoch": a.epochs, "loss": avg}, out / "best.pt")
