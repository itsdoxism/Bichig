...
def main():
    ...
    out = Path(a.out); out.mkdir(parents=True, exist_ok=True); tok.save(out / "vocab.json")
    print(f"device={device} lines={len(texts)} valid_lines={len(valid_texts)} tokens={len(ids)} vocab={len(tok)} params={sum(p.numel() for p in model.parameters()):,}")
    best_metric = float("inf")
    for epoch in range(1, a.epochs+1):
        model.train(); total=0.0
        for x,y in loader:
            x,y=x.to(device),y.to(device); opt.zero_grad(set_to_none=True)
            logits=model(x); loss=loss_fn(logits.reshape(-1, logits.size(-1)), y.reshape(-1)); loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step(); total += loss.item()
        avg=total/max(1,len(loader))
        val = evaluate_loss(model, valid_loader, loss_fn, device) if valid_loader else avg
        print(f"epoch={epoch:03d} loss={avg:.4f} ppl={math_exp(avg):.2f} val_loss={val:.4f} val_ppl={math_exp(val):.2f}")
        ckpt={"model":model.state_dict(),"config":cfg.to_dict(),"vocab":tok.itos,"epoch":epoch,"loss":avg,"val_loss":val}
